                 ...
                 int req_length, modbus_mapping_t *mb_mapping)
{
    ...
    /* Suppress any responses when the request was a broadcast unless quirk is enabled */
    return (slave == MODBUS_BROADCAST_ADDRESS &&
            !(ctx->quirks & MODBUS_QUIRK_BCAST_RESP)) ? 0 : send_msg(ctx, rsp, rsp_length);
}
